...
export const useResults = () => {
    ...
    const updateItem = React.useCallback(async (result: Omit<Result, 'id'>): Promise<Result> => {
        ...
        mutate(currentData => {
            ...
            const filteredData = currentData.filter(r => !(r.testId === newResult.testId && r.userId === newResult.userId));
...
export const useChatThreads = () => {
    ...
    const updateItem = React.useCallback(async (thread: ChatThread) => {
        const updatedThread = await DataActions.upsertChatThread(thread);

        mutate(currentData => {
            if (!currentData) return [updatedThread];
            const index = currentData.findIndex(t => t.id === updatedThread.id);
            if (index > -1) {
                const newData = [...currentData];
                newData[index] = updatedThread;
                return newData;
            } else {
                return [...currentData, updatedThread];
            }
        }, false);
    }, [mutate]);
